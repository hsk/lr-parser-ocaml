...
let step lex callback (input:string): (string * tokenizedInput) =
  ...
  let (_,r,rstr,rid) = lex |> ((0,None,"",-1) |> List.fold_left(
    fun (id,r,rstr,rid) ((token,pattern,priority,_) as rule) ->
      ...
      in
      match (m, r) with
      | (Some(str),None) -> (id+1,Some rule,str,id) (* 初めて見つかった場合 *)
      | (Some(str),Some(_,_,r_priority,_)) when priority > r_priority || (* 優先順位が高い *)
        (priority = r_priority && String.length str > String.length rstr) -> (* 同じ優先順のときは最長一致 *)
        (id+1,Some rule,str,id)
      | _ -> (id+1,r,rstr,rid) (* それ以外は以前のものを使用 *)
  )) in
  match r with
  | None ->
  ...
  | Some(token,_,_,_) ->
    (Str.string_after input (String.length rstr), (token, callback(rid, rstr)))
