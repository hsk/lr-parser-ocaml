...
DFAを構成する最小単位の要素がclosureitemです。

```
S -> A B
```

以上のような文法があった場合、読み込み状態を `.` で表し、そこで現れる次の記号列を`[]`で括って表すと
...
LRアイテムを集めてDFA内の一つの状態を表すのがLRアイテム集合closuresetです。
LRアイテム集合を生成するにはLRアイテムのリストを受け取って展開したあと、アイテムと同様にハッシュを生成します。
最初の種となるアイテム集合は開始記号アイテム`(S' -> S $)` のみをもつアイテム集合を使い、以下のようにして生成します:

```
  genClosureItem db (-1) 0 ["EOF"]
```

Grammerdb.getRuleByIdは-1のルールIDに対して開始記号の規則を返すのでこのように呼び出します。

アイテム集合は１つの状態にまとめられるすべてのアイテムIDを集める必要があるので、.の次の記号を再帰的に探索することになります。再帰的な検索を行う際にはすでに検索したものは検索対象から外す必要があるので履歴は残さなくてはなりません。
検索リストに結果を追加しながらインデックスでループすることで探索を行います。

closuresetの生成処理は、以下の３つの処理を行います：

- ①アイテム集合内のアイテムをlookaheadsごとに分解してフラットにする
- ②アイテム列をループで検索し.の次の記号が非終端の場合クロージャ展開したアイテムを追加して追加がなくなるまで繰り返す
- ③バラバラに分かれているlookaheadsを１つにマージします。

アイテム集合内のlookaheadsを１つに保つことで同一性チェックを間違いなく行います。

クロージャ展開は以下の処理を行います:

- .以降の記号列にlookaheadsを加えた記号列を求める
- ファースト集合を求めてソートします。
- .の次の記号を左辺に持つすべての規則とファースト集合の記号からアイテムを生成しリストになければ追加します。
- このとき、lookaheadsは1つの状態を保つためファースト集合の記号は１つ１つバラバラに生成します。

詳細はソースを見てください。

### dfagenerator.ml DFAジェネレータ

DFA は、node のリストとして表現します。
node は、一つの 状態 closureSet と 遷移 edge を持ちます。
edge は記号をラベルとして node のインデックスを持つ Map であり、状態遷移図の辺を意味します。

DFAの生成はgenerateLR1DFAで行い

- ① 開始記号から生成したアイテム集合を元に種となるDFAを生成し
- ② DFA更新処理で後続のアイテム集合をDFAに追加していき更新がなくなるまで続けます。

DFA更新処理はupdateDFAで行い

- .を１つ進めたラベルからアイテム集合へのマップを生成してループします。
- DFAから同一のアイテム集合を持つindexを検索してなければDFAを辺が空のnodeで拡張し
- indexのラベルにノードに辺がなければラベルからindexへの辺を追加します。
...
- ① 配列を2重ループでアイテム集合をマージしながら削除履歴を残します。
- ② 削除履歴から削除部分のないDFAを作成しつつ古いindexから新しいindexへの写像を作ります。
- ③ エッジ内のインデックスを古いindexから新しいindexへ書き換えます。

アイテム集合のマージは、LR0の部分が同じならアイテム集合内のアイテムをそれぞれマージします。
アイテムのマージはlookaheadsのunionを取ってアイテムを作り直します。
