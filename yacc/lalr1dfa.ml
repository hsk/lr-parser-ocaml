...
let merge_item db ci1 ci2 =
  ...
  in
  genClosureItem db ci1.rule_id ci2.dot_index (Array.of_list (merge 0 0))
...
let merge_set db cs1 cs2: Closureset.closureSet =
  ...
  let new_set = Array.mapi (fun i a1 -> merge_item db a1 a2.(i)) a1 in
  Closureset.genClosureSet db new_set
